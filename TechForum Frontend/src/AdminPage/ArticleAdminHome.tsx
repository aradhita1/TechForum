...
const AdminArticleList: React.FC = () => {
    ...
    try {
      ...
      if (response.status === 200 && response2.status === 200) {
        ...
        if (Array.isArray(articlesData)) {
          const validCommentsData = Array.isArray(commentsData) ? commentsData : [];
          const articlesWithComments = articlesData.map((article) => {
            const filteredComments = validCommentsData.filter(
  ...
  }, [fetchLatestComments, comments, commentsnew]);
...
export default memo(AdminArticleList);
